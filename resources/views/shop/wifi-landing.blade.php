...
<head>
    ...
    <meta name="theme-color" content="{{ $restaurant->theme_hex ?? '#DB2929' }}">
    <title>{{ $restaurant->name ?? __('app.connecting') }}</title>
    @php
        $themeHex = $restaurant->theme_hex ?? '#DB2929';
        $themeRgb = $restaurant->theme_rgb ?? '219, 41, 41';
        $r = hexdec(substr(ltrim($themeHex, '#'), 0, 2));
        $g = hexdec(substr(ltrim($themeHex, '#'), 2, 2));
        $b = hexdec(substr(ltrim($themeHex, '#'), 4, 2));
        $darkR = max(0, min(255, intval($r * 0.7)));
        $darkG = max(0, min(255, intval($g * 0.7)));
        $darkB = max(0, min(255, intval($b * 0.7)));
        $themeDarkHex = sprintf("#%02x%02x%02x", $darkR, $darkG, $darkB);
    @endphp
    <style>
        * {
            margin: 0;
        ...
        body {
            ...
            background: linear-gradient(135deg, {{ $themeHex }} 0%, {{ $themeDarkHex }} 100%);
            min-height: 100vh;
            display: flex;
            align-items: center;
        ...
        .btn-primary:hover {
            ...
            box-shadow: 0 10px 20px rgba({{ $themeRgb }}, 0.4);
        }
        .btn-secondary {
            background: #e2e8f0;
            color: #4a5568;
