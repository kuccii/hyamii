...
{{-- ============================== --}}
<div class="absolute top-0 left-1/4 w-96 h-96 bg-gradient-to-tr from-[rgb({{ $restaurant->theme_rgb }})]/10 to-transparent blur-3xl opacity-40 dark:opacity-20 pointer-events-none rounded-full"></div>
<div class="absolute top-[800px] right-10 w-[500px] h-[500px] bg-gradient-to-br from-[rgb({{ $restaurant->theme_rgb }})]/5 to-transparent blur-3xl opacity-30 pointer-events-none rounded-full"></div>
        ...
        <div class="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
            ...
            <div class="flex-1 text-center lg:text-left relative z-10">
                ...
                <div class="inline-flex items-center gap-2.5 px-4 py-2 bg-white/70 dark:bg-gray-900/60 backdrop-blur-md rounded-full shadow-sm border border-gray-150 dark:border-gray-800 text-xs font-semibold text-gray-700 dark:text-gray-300 mb-8 transition-transform hover:scale-[1.02] font-label tracking-wider uppercase">
                    <span class="relative flex h-2 w-2">
                        <span class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style="background-color: rgb({{ $restaurant->theme_rgb }})"></span>
                        <span class="relative inline-flex rounded-full h-2 w-2" style="background-color: rgb({{ $restaurant->theme_rgb }})"></span>
                    </span>
                    @if($restaurant->short_description)
                        {{ $restaurant->short_description }}
                    @else
                        @lang('messages.experienceOurService')
                    @endif
                </div>
                ...
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-gray-900 dark:text-white leading-[1.12] tracking-tight">
                    Welcome to <br>
                    <span class="text-transparent bg-clip-text bg-gradient-to-r font-black" style="background-image: linear-gradient(to right, rgb({{ $restaurant->theme_rgb }}), rgb({{ $restaurant->theme_rgb }}/0.7))">
                        {{ $restaurant->name }}
                    </span>
                </h1>
                ...
                <p class="mt-6 text-base sm:text-lg text-gray-500 dark:text-gray-400 max-w-xl mx-auto lg:mx-0 leading-relaxed font-light">
                    @lang('messages.experienceOurService') Savor exceptional culinary creations prepared by masters of flavor, utilizing the freshest local ingredients.
                </p>

                {{-- Micro Meta Info Badges --}}
                <div class="mt-8 flex flex-wrap items-center justify-center lg:justify-start gap-4">
                    @if(isset($shopBranch) && $shopBranch->address)
                        <div class="flex items-center gap-2.5 px-4 py-2.5 bg-white/80 dark:bg-gray-900/50 backdrop-blur-md rounded-2xl border border-gray-150 dark:border-gray-800/85 text-xs sm:text-sm text-gray-600 dark:text-gray-400 shadow-sm transition-all hover:border-gray-300 dark:hover:border-gray-750 font-label">
                            <span class="material-symbols-outlined text-lg" style="color: rgb({{ $restaurant->theme_rgb }})">location_on</span>
                            <span class="truncate max-w-[220px] font-medium">{{ $shopBranch->address }}</span>
                        </div>
                    @endif
                    @if(isset($shopBranch) && $shopBranch->opening_time && $shopBranch->closing_time)
                        <div class="flex items-center gap-2.5 px-4 py-2.5 bg-white/80 dark:bg-gray-900/50 backdrop-blur-md rounded-2xl border border-gray-150 dark:border-gray-800/85 text-xs sm:text-sm text-gray-600 dark:text-gray-400 shadow-sm transition-all hover:border-gray-300 dark:hover:border-gray-750 font-label">
                            <span class="material-symbols-outlined text-lg" style="color: rgb({{ $restaurant->theme_rgb }})">schedule</span>
                            <span class="font-medium">{{ $shopBranch->opening_time }} - {{ $shopBranch->closing_time }}</span>
                        </div>
                    @endif
                </div>
                ...
                <div class="mt-8 flex flex-wrap items-center justify-center lg:justify-start gap-4">
                    @if($restaurant->phone_number)
                        <a href="tel:{{ $restaurant->phone_number }}"
                           ...
                           style="background-color: rgb({{ $restaurant->theme_rgb }}); box-shadow: 0 10px 25px -5px rgba({{ $restaurant->theme_rgb }}, 0.3);">
            ...
            <div class="flex-1 w-full max-w-md lg:max-w-none relative">
                ...
                <div class="absolute -bottom-6 -right-4 z-20 bg-white dark:bg-gray-950 p-4 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-850 flex items-center gap-3 animate-bounce" style="animation-duration: 5s;">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background-color: rgba({{ $restaurant->theme_rgb }}, 0.1)">
                        <span class="material-symbols-outlined text-xl" style="color: rgb({{ $restaurant->theme_rgb }})">local_shipping</span>
                    </div>
                    <div>
                ...
                {{-- Main Image Showcase with Glowing Halo --}}
                <div class="relative group">
                    <div class="absolute -inset-2 bg-gradient-to-tr from-[rgb({{ $restaurant->theme_rgb }})]/30 to-[rgb({{ $restaurant->theme_rgb }})]/10 rounded-3xl blur-2xl opacity-40 group-hover:opacity-60 transition duration-1000 group-hover:duration-200"></div>
                    
                    @if($heroImageUrl)
                        <div class="relative rounded-3xl overflow-hidden shadow-2xl border border-white/20">
                            <img src="{{ $heroImageUrl }}" alt="{{ $restaurant->name }}" class="w-full h-auto object-cover aspect-[4/3] transform transition duration-700 group-hover:scale-105" />
                        </div>
                    @else
                        <div class="relative rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-br" style="background: linear-gradient(135deg, rgb({{ $restaurant->theme_rgb }}/0.15), rgb({{ $restaurant->theme_rgb }}/0.03)); padding: 2.5rem; border: 1px solid rgba({{ $restaurant->theme_rgb }}, 0.15);">
                            <div class="aspect-[4/3] flex items-center justify-center">
                                <img src="{{ $restaurant->logoUrl }}" alt="{{ $restaurant->name }}" class="max-w-[55%] max-h-[55%] object-contain opacity-50 dark:opacity-40 filter drop-shadow-md transform transition duration-500 group-hover:scale-105" />
                            </div>
                        </div>
                    @endif
                </div>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div class="text-center mb-16 reveal">
            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-150/80 dark:bg-gray-900/50 backdrop-blur-md rounded-full text-xs font-semibold text-gray-600 dark:text-gray-400 mb-4 border border-gray-200/40 dark:border-gray-800/40">
                ...
                @lang('menu.howItWorks')
            </span>
            <h2 class="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">@lang('messages.elevateEveryTouchpoint')</h2>
        </div>

        {{-- Modern Minimalist Cards Grid --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            @foreach($steps as $step)
                <div class="group relative bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-150 dark:border-gray-850 hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-xl transition-all duration-300 reveal flex flex-col justify-between">
                    ...
                    {{-- Transparent Floating Large Step Number --}}
                    <div class="absolute top-6 right-8 text-5xl font-black text-gray-105 dark:text-gray-800/30 select-none font-hanken group-hover:text-[rgb({{ $restaurant->theme_rgb }})]/15 transition duration-300">
                        {{ $step['num'] }}
                    </div>
                    ...
                    <div>
                        ...
                        <div class="w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 shadow-sm"
                            ...
                            <span class="material-symbols-outlined text-2xl transition duration-350 group-hover:scale-110" style="color: rgb({{ $restaurant->theme_rgb }})">{{ $step['icon'] }}</span>
                        </div>
                        
                        {{-- Card Header --}}
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-3 group-hover:text-[rgb({{ $restaurant->theme_rgb }})] transition duration-200">{{ $step['title'] }}</h3>
                        
                        {{-- Card Desc --}}
                        <p class="text-sm text-gray-500 dark:text-gray-400 leading-relaxed font-light">{{ $step['desc'] }}</p>
                    </div>
                </div>
            @endforeach
        </div>
...
<section class="py-20 sm:py-24 relative overflow-hidden">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="rounded-3xl p-8 sm:p-14 lg:p-20 text-center relative overflow-hidden reveal shadow-xl border"
             style="background: radial-gradient(circle at top right, rgb({{ $restaurant->theme_rgb }}/0.15), transparent 60%), linear-gradient(135deg, rgb({{ $restaurant->theme_rgb }}/0.05), rgb({{ $restaurant->theme_rgb }}/0.01)); border-color: rgba({{ $restaurant->theme_rgb }}, 0.15);">
            
            {{-- Absolute blur glows for aesthetic luxury feel --}}
            <div class="absolute -bottom-16 -left-16 w-48 h-48 bg-[rgb({{ $restaurant->theme_rgb }})]/10 rounded-full blur-2xl pointer-events-none"></div>
            ...
            <div class="relative z-10">
                ...
                <h2 class="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-white tracking-tight leading-tight max-w-2xl mx-auto">
                    ...
                    <span class="text-transparent bg-clip-text bg-gradient-to-r font-black" style="background-image: linear-gradient(to right, rgb({{ $restaurant->theme_rgb }}), rgb({{ $restaurant->theme_rgb }}/0.7))">Visit us or place your order online!</span>
                </h2>
                
                <p class="mt-4 text-gray-500 dark:text-gray-400 max-w-md mx-auto font-light">
                    @lang('messages.experienceOurService') Experience our high quality dishes in a gorgeous premium atmosphere.
                </p>
                ...
                <div class="mt-10 flex flex-wrap justify-center gap-4">
                    @if(isset($shopBranch) && $shopBranch->address)
                        <a href="https://www.google.com/maps/search/?api=1&query={{ urlencode($shopBranch->address) }}" target="_blank"
                           ...
                           style="background-color: rgb({{ $restaurant->theme_rgb }}); color: white; box-shadow: 0 10px 25px -5px rgba({{ $restaurant->theme_rgb }}, 0.35);">
...
<footer class="py-16 sm:py-20 border-t border-gray-150 dark:border-gray-850 bg-white dark:bg-gray-950">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            ...
            <div class="lg:col-span-1">
                <div class="flex items-center gap-3.5 mb-6">
                    <img src="{{ $restaurant->logoUrl }}" class="h-10 w-10 rounded-xl object-cover shadow-sm border border-gray-100 dark:border-gray-800" alt="" />
                    <span class="text-xl font-bold text-gray-900 dark:text-white tracking-tight">{{ $restaurant->name }}</span>
                </div>
            ...
            {{-- Quick Links --}}
            <div>
                <h4 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-widest mb-6">@lang('messages.quickLinks')</h4>
                <ul class="space-y-4 text-sm text-gray-500 dark:text-gray-400">
                    <li><a href="{{ route('shop_restaurant', [$restaurant->hash]) }}" class="hover:text-[rgb({{ $restaurant->theme_rgb }})] transition duration-200">@lang('menu.home')</a></li>
                    <li><a href="{{ route('shop_about', [$restaurant->hash]) }}" class="hover:text-[rgb({{ $restaurant->theme_rgb }})] transition duration-200">@lang('menu.about')</a></li>
                    <li><a href="{{ route('shop_contact', [$restaurant->hash]) }}" class="hover:text-[rgb({{ $restaurant->theme_rgb }})] transition duration-200">@lang('menu.contact')</a></li>
                </ul>
            </div>

            {{-- Contact Info --}}
            <div>
                <h4 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-widest mb-6">@lang('app.contact')</h4>
                <ul class="space-y-4 text-sm text-gray-500 dark:text-gray-400">
                    @if($restaurant->email)
                        <li class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-base" style="color: rgb({{ $restaurant->theme_rgb }})">mail</span>
                            <a href="mailto:{{ $restaurant->email }}" class="hover:text-[rgb({{ $restaurant->theme_rgb }})] transition duration-200 truncate">{{ $restaurant->email }}</a>
                        </li>
                    @endif
                    @if($restaurant->phone_number)
                        <li class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-base" style="color: rgb({{ $restaurant->theme_rgb }})">call</span>
                            <a href="tel:{{ $restaurant->phone_number }}" class="hover:text-[rgb({{ $restaurant->theme_rgb }})] transition duration-200">{{ $restaurant->phone_number }}</a>
                        </li>
                    @endif
                </ul>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    @if ($restaurant->twitter_link)
                        <a href="{{ $restaurant->twitter_link }}" target="_blank" class="w-10 h-10 rounded-xl bg-gray-50 dark:bg-gray-900 flex items-center justify-center text-gray-400 hover:text-white dark:text-gray-400 hover:bg-[rgb({{ $restaurant->theme_rgb }})] dark:hover:bg-[rgb({{ $restaurant->theme_rgb }})] transition-all duration-300 border border-gray-150 dark:border-gray-850 shadow-sm" aria-label="Twitter">
        ...
        <div class="pt-8 border-t border-gray-150 dark:border-gray-850 text-center text-xs text-gray-400 dark:text-gray-600 font-light">
            <p>&copy; {{ now()->year }} {{ $restaurant->name }}. @lang('app.allRightsReserved')</p>
        </div>
    </div>
</footer>
