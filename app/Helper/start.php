...
function restaurant()
{
    ...
    if (user()) {
        if (user()->restaurant_id) {
            session(['restaurant' => Restaurant::with(['currency', 'branches'])->find(user()->restaurant_id)]);
            return session('restaurant');
        }
    }
...
function dateFormat()
{
    ...
    if ($user && $user->restaurant_id) {
        $restaurant = restaurant() ?: Restaurant::find($user->restaurant_id);
...
if (!function_exists('clearRestaurantContextCache')) {

    function clearRestaurantContextCache(?int $restaurantId = null): void
    {
        session()->forget([
            'restaurant',
            'branch',
        ...
        if ($restaurantId) {
            clearRestaurantModulesCache($restaurantId);
        }
    }
}
