...
class ShopController extends Controller
{
    ...
    /**
     * Get the hero cover image URL from CartHeaderSetting
     */
    private function getHeroImageUrl(?Restaurant $restaurant): ?string
    {
        if (!$restaurant) return null;

        $restaurant->load('cartHeaderSetting.images');
        $setting = $restaurant->cartHeaderSetting;

        if (!$setting || $setting->is_header_disabled || $setting->header_type !== 'image') {
            return null;
        }

        return $setting->images->first()?->image_url;
    }
    ...
    public function cart(string $hash)
    {
        ...
        $heroImageUrl = $this->getHeroImageUrl($restaurant);
    ...
    public function bookTable(string $hash)
    {
        ...
        return view('shop.book_a_table', compact('restaurant', 'shopBranch', 'heroImageUrl'));
    }
    ...
    public function myBookings(string $hash)
    {
        ...
        return view('shop.bookings', compact('restaurant', 'shopBranch', 'heroImageUrl'));
    }
    ...
    public function myAddresses(string $hash)
    {
        ...
        return view('shop.addresses', compact('restaurant', 'shopBranch', 'heroImageUrl'));
    }
    ...
    public function profile(string $hash)
    {
        ...
        return view('shop.profile', compact('restaurant', 'shopBranch', 'heroImageUrl'));
    }
    ...
    public function myOrders(string $hash)
    {
        ...
        return view('shop.orders', compact('restaurant', 'shopBranch', 'heroImageUrl'));
    }
    ...
    public function about(string $hash)
    {
        $restaurant = Restaurant::with('currency')->where('hash', $hash)->firstOrFail();
        $shopBranch = $this->getShopBranch($restaurant);
        $modules = $this->getPackageModules($restaurant);
        $heroImageUrl = $this->getHeroImageUrl($restaurant);

        $this->redirectIfSubdomainIsEnabled($restaurant);

        return view('shop.about', compact('restaurant', 'shopBranch', 'modules', 'heroImageUrl'));
    }
    ...
    public function contact(string $hash)
    {
        ...
        return view('shop.contact', compact('restaurant', 'shopBranch', 'modules', 'heroImageUrl'));
    }
